...
// This endpoint creates initial admin user and sample products
export async function GET() {
  try {
    ...
    const products = [
      ...
      {
        name: JSON.stringify({ ro: "Þuicã de miere", en: "Honey Brandy" }),
        description: JSON.stringify({
    ...
    return NextResponse.json({
      ...
      productsCreated: existingProductsCount === 0 ? products.length : 0
    })
  } catch (error) {
